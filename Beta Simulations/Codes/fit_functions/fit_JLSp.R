#### Fit JLSp To A Dataset ####

fit.JLSp <- function(beta_vals, metadata, expVar, coVars){
  
  #### Tracking time starts ####
  
  start.time <- Sys.time()
  
  #### Standard JLSp pipeline for 2 groups ####
  ...
  mem <- peakRAM::peakRAM({
    ...
    spe <- function(x){
      ...
      fit <- jlst::jlsp(y = x, x = grpvar, var.type = 1, covar = NULL)
      resTab <- data.frame(Pval = fit[["location_scale_test"]]$P)
      return(resTab)
    }
