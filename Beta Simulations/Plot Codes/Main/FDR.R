packages <- c('ggplot2', 'stringr', "reshape2", "forcats", "openxlsx",
...
for(k in 1:length(out_pc_list)){
  ...
  for(j in 1:length(grp_split)){
    ...
    write.xlsx(summary_FDR, file = paste(dir, "Plots/Main/Excel", 
...
for (i in seq_along(batches_FDR)) {
  output_file <- paste(dir, "Plots/Main", paste0(paste("FDR_vareff", var_eff, 'pDM', p_DM, sep = '_'), ".png"), sep = '/')
